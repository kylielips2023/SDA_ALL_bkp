...
from my_mod import vpn_sql_van
from my_mod import cnf_bvb_van
from my_mod import api_mysql_van
...
file_vpn_dead=cnf_bvb_van.file_vpn_dead
p_vpn_dead=cnf_bvb_van.p_vpn_dead
...
def get_random_vpn():
	...
	# fresh_config,int_used=vpn_sql.get_fresh_config(typ0)
	# print(fresh_config,int_used)
	data_id,data_cnf_names,data_used,van_config_left = api_mysql_van.get_random_api_van()
...
def fnc_vpn():
	...
	with open ('/var/log/openvpn/openvpn.log', "r") as logfile:
		...
		if logfile.read().find('Sequence Completed') !=-1:
			...
			api_mysql_van.update_conf_van_api(id_config)
			# print(meddas)
			cnf_bvb_van.ap_2_l0g(meddas)
			cnf_bvb_van.alias_send_msg(meddas)
			# print ("  OK !!!!!")
		...
		if logfile.read().find('AUTH_FAILED'):
			...
			if len(retry_count) ==4:
				cnf_bvb_van.alias_send_msg("AUTH_FAILED")
