...
def get_random_vpn():
	...
	data_id,data_cnf_names,data_used = api_mysql_van.get_random_api()
...
def fnc_vpn():
	...
	int_used=api_mysql_van.count_left_api()
	...
	with open ('/var/log/openvpn/openvpn.log', "r") as logfile:
		...
		if logfile.read().find('Sequence Completed') !=-1:
			...
			api_mysql_van.update_conf_nord_api(id_config)
			print(meddas)
